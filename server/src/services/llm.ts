...
export async function generateChatResponse(
  ...
  // Build schema descriptions for all datasets
  const datasetDescriptions = datasets
    .map((ds) => {
      const cols = ds.schema
        ...
        .join('\n')
      return `Table: "${ds.table_name}" (source file: "${ds.name}", ${ds.row_count} rows)\nColumns:\n${cols}`
    })
    .join('\n\n')

  const tableNames = datasets.map((ds) => `"${ds.table_name}"`).join(', ')
...
- Available tables: ${tableNames}. Always double-quote table and column names.
- You can JOIN across tables using explicit table prefixes (e.g. "table_a"."col" = "table_b"."col") when the user asks about relationships between datasets.
