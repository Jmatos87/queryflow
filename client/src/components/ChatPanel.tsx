...
import { Send, Loader2, Table2, Bot, User } from 'lucide-react'
...
export function ChatPanel() {
  ...
  const messages = useChatStore((s) =>
    datasetId ? (s.messagesByDataset[datasetId] ?? EMPTY_MESSAGES) : EMPTY_MESSAGES
  )
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      ...
      <div className="flex items-center gap-2 border-b px-4 py-3">
        <h2 className="text-sm font-semibold truncate">{activeDataset.name}</h2>
        ...
        <Badge variant="outline" className="text-[10px]">
          {activeDataset.schema.length} cols
        </Badge>
      </div>
            ...
            <div className="flex gap-3">
              ...
              <div className="max-w-[80%] rounded-2xl bg-muted px-4 py-2.5 text-sm">
                <p className="leading-relaxed">
                  ...
                  <strong>{activeDataset.name}</strong>. This dataset has{' '}
                  {activeDataset.row_count.toLocaleString()} rows and{' '}
                ...
                <ul className="mt-1 list-inside list-disc text-muted-foreground">
                  ...
                  <li>What does the {activeDataset.schema[0]?.name} column represent?</li>
                </ul>
              </div>
            </div>
          )}
